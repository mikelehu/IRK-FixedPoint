...
# line 32 "quad-Gausstm.c"


void quadGauss P(( int _tp1, int _tp2, int _tp3, double _tp4, double _tp5, const unsigned char * _tp6, int _tpl6, const unsigned char * _tp7, int _tpl7, double _tp8, const unsigned char * _tp9, int _tpl9, int * _tp10, long _tpl10, int _tp11, int _tp12, int _tp13, int _tp14, int _tp15, const char * _tp16, int _tp17, int _tp18));
...
#endif
{
	...
	double _tp4;
	double _tp5;
	const unsigned char * _tp6;
	int _tpl6;
	const unsigned char * _tp7;
	int _tpl7;
	double _tp8;
	const unsigned char * _tp9;
	int _tpl9;
	int * _tp10;
	long _tpl10;
	int _tp11;
	...
	if ( ! MLGetByteString( mlp, &_tp6, &_tpl6, MLSpecialCharacter) ) goto L5;
	if ( ! MLGetByteString( mlp, &_tp7, &_tpl7, MLSpecialCharacter) ) goto L6;
	if ( ! MLGetReal( mlp, &_tp8) ) goto L7;
	if ( ! MLGetByteString( mlp, &_tp9, &_tpl9, MLSpecialCharacter) ) goto L8;
	...
	if ( ! MLGetString( mlp, &_tp16) ) goto L15;
	if ( ! MLGetInteger( mlp, &_tp17) ) goto L16;
	if ( ! MLGetInteger( mlp, &_tp18) ) goto L17;
	if ( ! MLNewPacket(mlp) ) goto L18;

	quadGauss(_tp1, _tp2, _tp3, _tp4, _tp5, _tp6, _tpl6, _tp7, _tpl7, _tp8, _tp9, _tpl9, _tp10, _tpl10, _tp11, _tp12, _tp13, _tp14, _tp15, _tp16, _tp17, _tp18);

	res = 1;
L18: L17: L16:	MLReleaseString(mlp, _tp16);
L15: L14: L13: L12: L11: L10:	MLReleaseInteger32List( mlp, _tp10, _tpl10);
L9:	MLReleaseByteString( mlp, _tp9, _tpl9);
L8: L7:	MLReleaseByteString( mlp, _tp7, _tpl7);
L6:	MLReleaseByteString( mlp, _tp6, _tpl6);
L5: L4: L3: L2: L1: 
L0:	return res;
} /* _tr0 */
...
static const char* evalstrs[] = {
	"quadGauss::usage = \"irteera = quadGauss[neq_Integer, n_Integer, ",
	"ns_Real, t0_Real, t1_Real, dat_String,els_String, h_Real, rpars_",
	"String, ipars_List,approx_Integer, threads_Integer, alg_Integer,",
	" rdigits_Integer, filename_String, sampling_Integer, codfun_Inte",
	"ger] \"",
	(const char*)0,
	(const char*)0
};
...
#endif
{
	int _res;
	_res = MLConnect(mlp);
	if (_res) _res = _definepattern(mlp, (char *)"quadGauss[neq_Integer, n_Integer, ns_Integer, t0_Real, t1_Real, dat_String, els_String, h_Real, rpars_String, ipars_List, approx_Integer, threads_Integer, alg_Integer, rdigits1_Integer, rdigits2_Integer, filename_String, sampling_Integer, codfun_Integer ]", (char *)"{neq,n,ns,t0,t1,dat,els, h,rpars,ipars,approx,threads,alg,rdigits1,rdigits2,filename, sampling,codfun}", 0);
